#!/bin/bash
# This is the MySQL DB seed script for the production environment.
...
if [ -f ./sql/prod-dbs.sql.gz ]; then
  echo "prod-dbs.sql.gz found. Extracting..."
  gunzip ./sql/prod-dbs.sql.gz
elif [ -f ./sql/prod-dbs.sql ]; then
  echo "unzipped prod-dbs.sql found."
else
  echo "prod-dbs.sql.gz not found. Download it from the Narrative Dropbox."
  exit 1
fi
...
if mysql -h 127.0.0.1 -P 3306 -u ${MYSQL_ROOT_USER} --password=${MYSQL_ROOT_PASSWORD} -e "show databases;" | grep global; then
  ...
  mysql -h 127.0.0.1 -P 3306 -u ${MYSQL_ROOT_USER} --password=${MYSQL_ROOT_PASSWORD} < /opt/narrative-platform/sql/prod-dbs.sql
fi
...
if mysql -h 127.0.0.1 -P 3306 -u global_user --password=${MYSQL_GLOBAL_PASSWORD} -e "use global; show tables"; then
  ...
  envsubst < /opt/narrative-platform/mysql/prod-users.sql > /mysql-user.sql
...
if mysql -h 127.0.0.1 -P 3306 -u ${MYSQL_ROOT_USER} --password=${MYSQL_ROOT_PASSWORD} -e "use mysql; show procedure status;" | grep global_user; then
  ...
  mysql -h 127.0.0.1 -P 3306 -u ${MYSQL_ROOT_USER} --password=${MYSQL_ROOT_PASSWORD} < /opt/narrative-platform/sql/prod-stored-procedures.sql
fi
