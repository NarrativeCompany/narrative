 ...
 * @author brian
 */
public class ASCIIFoldingUtil {
    public static String getAsciiFoldedString(String text) {
        String ret = "";
        int length = text.length();
        for (int i = 0; i < length; i++) {
            ret += getAsciiFoldedChar(text.charAt(i));
        }
        return ret;
    }

    /**
     * bl: based off of the ASCIIFoldingFilter from Lucene. it handles some, but definitely not all, of the special
     * unicode characters and cyrillics that have visual ASCII counterparts.
     *
     * @param c the character to analyze
     * @return an ASCII string representation of the character, if any. otherwise, just returns the original character as a string.
     */
    private static String getAsciiFoldedChar(char c) {
        if (c < '\u0080') {
