...
public class ImportPlugin implements TagPlugin {
    ...
    public void doTag(TagPluginContext ctxt) {
        ...
        ctxt.generateJavaSource("        String " + servletPathName + " = " + "((HttpServletRequest)_jspx_page_context.getRequest()).getServletPath();");
        ...
        if (hasVarReader) {
            ...
            ctxt.generateJavaSource("    _jspx_page_context.setAttribute(" + varReaderName + ", " + tempReaderName + ");");

            //if the url is relative
            ctxt.generateJavaSource("}else{");

            //if the url is relative, http request is needed
            ctxt.generateJavaSource("    if (!(_jspx_page_context.getRequest() instanceof HttpServletRequest  " + "&& _jspx_page_context.getResponse() instanceof HttpServletResponse)){");
            ...
            if (hasContext) {
                ...
                ctxt.generateJavaSource("        " + servletContextName + " = _jspx_page_context.getServletContext().getContext(" + contextName + ");");
                ctxt.generateJavaSource("    }else{");
            ...
            ctxt.generateJavaSource("    org.apache.jasper.tagplugins.jstl.Util.ImportResponseWrapper " + irwName + " = new org.apache.jasper.tagplugins.jstl.Util.ImportResponseWrapper((HttpServletResponse) _jspx_page_context.getResponse());");
            ctxt.generateJavaSource("    if(" + charSetName + " == null){");
            ctxt.generateJavaSource("        " + charSetName + " = org.apache.jasper.tagplugins.jstl.Util.DEFAULT_ENCODING;");
            ctxt.generateJavaSource("    }");
            ctxt.generateJavaSource("    " + irwName + ".setCharEncoding(" + charSetName + ");");
            ctxt.generateJavaSource("    try{");
            ctxt.generateJavaSource("        " + requestDispatcherName + ".include(_jspx_page_context.getRequest(), " + irwName + ");");
            ...
            //close the reader
            ctxt.generateJavaSource("java.io.Reader " + tempReaderName2 + " = (java.io.Reader)_jspx_page_context.getAttribute(" + varReaderName + ");");
        ...
        //if the varReader is not specified
            ...
            if (hasVar) {
                ...
                ctxt.generateJavaSource("_jspx_page_context.setAttribute(\"" + strVar + "\"," + tempStringName + "," + iScope + ");");
            } else {
