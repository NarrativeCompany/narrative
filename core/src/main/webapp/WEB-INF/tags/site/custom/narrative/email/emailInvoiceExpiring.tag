...
<ss:ref var="task" className="org.narrative.network.customizations.narrative.niches.nicheauction.services.SendInvoiceExpiringEmail"/>
${ss:assert(task.invoice.type.nicheAuction, 'This task is only setup to create the email for a niche purchase currently.')}
...
<n_email:emailWrapper
        subject="${h:wordlet1Arg('jsp.site.custom.narrative.email.emailInvoiceExpiring.subject', niche.nameForHtml)}"
        actionText="${h:wordlet('jsp.site.custom.narrative.email.emailInvoiceExpiring.payInvoice')}"
        actionUrl="${task.invoice.displayUrl}">
    <e:elt name="p">
        <ss:set var="nicheLink">
            <e:a href="${niche.displayUrl}">${niche.nameForHtml}</e:a>
        </ss:set>
        ${h:wordlet1Arg('jsp.site.custom.narrative.email.emailInvoiceExpiring.intro', nicheLink)}
    </e:elt>
</n_email:emailWrapper>